...
{
    public byte[] Generate(Payment payment, Invoice invoice, string language = "en", string? clinicName = null)
    {
        ...
        return Document.Create(document => document.Page(page =>
    {
        ...
        page.Content().Column(column =>
        {
            ...
            column.Item().Text(clinicName ?? invoice.ClinicName).FontSize(20).Bold().FontColor(PdfBrand.Navy);
            column.Item().BorderBottom(2).BorderColor(PdfBrand.Primary).PaddingBottom(12).Text(payment.Kind == "Refund" ? l.Text("REFUND RECEIPT", "إيصال استرداد") : l.Text("PAYMENT RECEIPT", "إيصال دفع")).Bold();
            ...
            column.Item().Background(PdfBrand.Background).Padding(12).Text($"{l.Text("Amount", "المبلغ")}: {l.Number(Math.Abs(payment.Amount))} {payment.Currency}").FontSize(16).Bold();
