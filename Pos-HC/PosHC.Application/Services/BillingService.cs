...
namespace PosHC.Application.Services
{
    public class BillingService : IBillingService
    {
        ...
        public BillingService(IClinicStore store, IClinicSettingsService settingsService, ICashShiftService cashShiftService, IAuditService auditService, ICurrentStaff staff)
        {
            _store = store;
        ...
        {
            var invoice = await _store.Find<Invoice>(x => x.Id == id, ct) ?? throw new BusinessException("Invoice not found.", 404);
            invoice.Items = await _store.List<InvoiceItem>(x => x.InvoiceId == id, int.MaxValue, ct: ct);
            return invoice;
        }
        public async Task<InvoiceBalance> Balance(Guid id, CancellationToken ct)
        {
            var invoice = await Invoice(id, ct);
            var paid = (await _store.List<Payment>(x => x.InvoiceId == id, int.MaxValue, ct: ct)).Sum(x => x.Amount);
            var credits = (await _store.List<CreditNote>(x => x.InvoiceId == id, int.MaxValue, ct: ct)).Sum(x => x.Amount);
        ...
        public Task<Invoice> Create(CreateInvoiceDto input, CancellationToken ct) => _store.Transaction(async () =>
        {
            if (input.RequestId.HasValue && await _store.Find<Invoice>(x => x.RequestId == input.RequestId, ct) is { } existing)
            {
            ...
            var doctor = await _store.Find<Doctor>(x => x.Id == doctorId && x.IsActive, ct) ?? throw new BusinessException("Select an active doctor.");
            var patient = await _store.Find<Patient>(x => x.Id == input.PatientId && x.IsActive, ct) ?? throw new BusinessException("Select an active patient.");
            ...
            {
                Id = Guid.NewGuid(),
                Number = await _store.NextInvoiceNumber(ct),
            ...
            {
                VisitNotes.Apply(invoice, new VisitNotesInput(input.VisitDescription, input.Diagnosis), _staff);
            }
            ...
            foreach (var item in input.Items!)
            {
                ...
                var catalog = await _store.Find<CatalogItem>(x => x.Id == item.CatalogItemId && x.IsActive, ct) ?? throw new BusinessException("Select an active service or product.");
            ...
            _store.Add(invoice);
            _auditService.Record("Create", "Invoice", invoice.Id, $"{invoice.Number}; {invoice.Currency}; {invoice.Total}");
            return invoice;
        }, ct);
        public Task<Invoice> ChangeStatus(Guid id, string status, string reason, CancellationToken ct) => _store.Transaction(async () =>
        {
            ...
            _auditService.Record(status, "Invoice", id, reason);
            return invoice;
        }, ct);
        public Task<Payment> Collect(PaymentRequestDto input, CancellationToken ct) => _store.Transaction(async () =>
        {
            if (input.RequestId.HasValue && await _store.Find<Payment>(x => x.RequestId == input.RequestId, ct) is { } existing)
            {
            ...
            Check(await _store.Find<PaymentType>(x => x.Id == input.PaymentTypeId, ct) != null, "Payment type not found.");
            ...
            {
                shiftId = (await _store.Find<CashShift>(x => x.UserId == _staff.Id && x.Currency == balance.Invoice.Currency && x.ClosedAt == null, ct) ?? throw new BusinessException("Open a cash shift for this currency first.")).Id;
            }
            ...
            _store.Add(row);
            _auditService.Record("Collect", "Invoice", input.InvoiceId, $"{amount} {row.Currency}; payment={row.Id}");
            return row;
        }, ct);
        public Task<CreditNote> Credit(Guid id, decimal amount, string reason, Guid requestId, CancellationToken ct) => _store.Transaction(async () =>
        {
            Check(requestId != Guid.Empty, "Request ID is required.");
            if (await _store.Find<CreditNote>(x => x.RequestId == requestId, ct) is { } existing)
            {
                return existing;
            }
            ...
            _store.Add(row);
            _auditService.Record("Credit", "Invoice", id, $"{amount}; {row.Reason}");
            return row;
        }, ct);
        public Task<Payment> Refund(Guid id, decimal amount, int method, string reason, Guid requestId, CancellationToken ct) => _store.Transaction(async () =>
        {
            Check(requestId != Guid.Empty, "Request ID is required.");
            if (await _store.Find<Payment>(x => x.RequestId == requestId, ct) is { } existing)
            {
                return existing;
            }
            ...
            if (method == 1)
            {
                var shift = await _store.Find<CashShift>(x => x.UserId == _staff.Id && x.Currency == balance.Invoice.Currency && x.ClosedAt == null, ct) ?? throw new BusinessException("Open a cash shift for this currency first.");
                Check(await _cashShiftService.Expected(shift, ct) >= amount, "Refund exceeds available cash.");
                shiftId = shift.Id;
            }
            ...
            _store.Add(row);
            _auditService.Record("Refund", "Invoice", id, $"{amount}; {row.Reference}");
            return row;
        }, ct);
    }
}
